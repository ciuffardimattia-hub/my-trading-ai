...
LANGUAGES = {
    IT {
        ...
        about_h Perché Market-Core, about_p Il nostro Hub fonde i dati live con la potenza di Google Gemini per un'analisi professionale 247.,
    ...
    EN {
        ...
        about_h Why Market-Core, about_p Our Hub merges live data with Google Gemini for professional 247 analysis.,
    ...
    ES {
        ...
        about_h ¿Por qué Market-Core, about_p Nuestro Hub fusiona datos en vivo con la potencia de Google Gemini para un análisis profesional 247.,
    ...
    FR {
        ...
        about_h Pourquoi Market-Core, about_p Notre Hub fusionne les données avec la puissance de Google Gemini pour une analyse 247.,
...
st.markdown(
    ...
    .hero-title { font-size clamp(50px, 8vw, 80px); font-weight 900; text-align center; background linear-gradient(90deg, #00ff41, #ff00ff); -webkit-background-clip text; -webkit-text-fill-color transparent; margin-bottom0; }
    ...
    .stButtonbutton { background transparent !important; color #00ff41 !important; border 2px solid #00ff41 !important; width 100%; font-weight bold; }
    .stButtonbuttonhover { background #00ff41 !important; color black !important; box-shadow 0 0 15px #00ff41; }
...
def fetch_news_rss(q)
    news = []
    try
        url = fhttpsnews.google.comrsssearchq={q}+stock+market&hl=it&gl=IT
...
if API_KEY
    ...
    try
        ...
        target_model = None
        for m_name in ['modelsgemini-1.5-flash', 'modelsgemini-1.5-pro', 'modelsgemini-1.0-pro', 'modelsgemini-pro']
...
if st.session_state.page == landing
    ...
    st.markdown(fdiv class='hero-title'{L['hero_t']}div, unsafe_allow_html=True)
    st.markdown(fp style='text-align center; color #888; font-size20px;'{L['hero_s']}p, unsafe_allow_html=True)
    st.markdown(fdiv class='about-section'h2{L['about_h']}h2p{L['about_p']}pdiv, unsafe_allow_html=True)
    
    col_f1, col_f2, col_f3 = st.columns(3)
    col_f1.markdown(fdiv class='feat-card'h3{L['feat_ia']}h3p{L['feat_ia_p']}pdiv, unsafe_allow_html=True)
    col_f2.markdown(fdiv class='feat-card' style='border-color#ff00ff;'h3{L['feat_cloud']}h3p{L['feat_cloud_p']}pdiv, unsafe_allow_html=True)
    col_f3.markdown(fdiv class='feat-card'h3{L['feat_turbo']}h3p{L['feat_turbo_p']}pdiv, unsafe_allow_html=True)
    ...
    st.markdown(h2 style='text-align center; color #ff00ff;'Autenticazione Nodoh2, unsafe_allow_html=True)
    ...
    if not st.session_state.terminal_booted
        ...
        with boot_placeholder.container()
            st.markdown(div style='margin-top20vh; font-family Courier New;', unsafe_allow_html=True)
            for phrase in boot_phrases
                st.markdown(fh4 style='color#00ff41;'{phrase}h4, unsafe_allow_html=True)
    ...
    with st.sidebar
        st.markdown(fh2 style='color#ff00ff;'{L['hero_t']}h2, unsafe_allow_html=True)
        ...
        if st.session_state.portfolio
            st.markdown(fspan style='color#888;'{L['port_subtitle']}span, unsafe_allow_html=True)
            for item in st.session_state.portfolio
                ticker, price, qty = item[1], float(item[2]), float(item[3])
                
                col_txt, col_btn = st.columns([4, 1])
                with col_txt
                    st.markdown(fdiv class='asset-box'b{ticker}bbr{qty} qt  ${price.2f}div, unsafe_allow_html=True)
                with col_btn
                    if st.button(🗑️, key=fdel_{ticker})
    ...
    st.markdown(fh3 style='text-aligncenter;'🔍 {L['main_search']}h3, unsafe_allow_html=True)
                        ...
                        if model
                            ...
                            try
                                ...
                                if st.session_state.portfolio
                                    ...
                                    port_context = fPortafoglio attuale dell'utente {asset_list}.

                                ctx = fDati attuali - Asset {t_sym}, Prezzo {df['Close'].iloc[-1].2f}, RSI {df['RSI'].iloc[-1].1f}. {port_context}
                                
                                # GESTIONE MEMORIA CONTESTO (Ultimi 4 scambi per non sovraccaricare le API)
                                history = 
                                if len(st.session_state.msgs)  1
                                    history = Contesto della conversazione precedenten + n.join([f{m['role']} {m['content']} for m in st.session_state.msgs[-5-1]])
                                
                                prompt = fSei un analista finanziario IA del Market-Core.n{ctx}n{history}nL'utente chiede {inp}nRispondi in modo professionale e schematico in lingua {st.session_state.lang}.
                                ...
                                terminal_placeholder.error(fErrore tecnico IA {e})
                        else st.error(L'IA non è al momento disponibile.)

st.markdown(fdiv style='text-aligncenter; color#444; font-size10px; margin-top50px;'{L['disclaimer']}div, unsafe_allow_html=True)
